...
(require (except-in tasklog %append))
...
(define %length
  (%rel (h t n m)
    (('() 0))
    (((cons h t) n) (%length t m) (%is n (m) (+ m 1)))))
...
(define %count
  (letrec ((countaux
	     (%rel (m n m+1 x y z)
	       (('() m m))
	       (((cons x y) m n)
                (%delete x y z) (%is m+1 (m) (+ m 1)) (countaux z m+1 n)))))
    (%rel (x n)
      ((x n) (countaux x 0 n)))))
...
(define %fact*
  (%rel (n n! n-1 n-1!)
    ((0 1))
    ((n n!) (%is n-1 (n) (- n 1)) (%fact n-1 n-1!) (%is n! (n n-1!) (* n n-1!)))))
...
(define %fact
  (letrec ((factaux
	     (%rel (n! m x m-1 xx)
	       ((0 n! n!))
               ((m x n!) (%is m-1 (m) (- m 1)) (%is xx (x m) (* x m))
		(factaux m-1 xx n!)))))
    (%rel (n n!)
      ((n n!) (factaux n 1 n!)))))
